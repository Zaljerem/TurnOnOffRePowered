...
{    
    ...
    public static class TurnItOnUtility
    {
        ...
        public static void DefsLoaded()
        {
            ...
            if (rimfactoryIsLoaded)
            {
                ...
                rimfactoryAssemblerDefs = DefDatabase<ThingDef>.AllDefsListForReading
                    .Where(def => def.defName.StartsWith("PRF_") && def.thingClass.Name.EndsWith("Assembler")).ToList();
            }
        ...
        {
            if (powerLevels.ContainsKey(defName)) return;
            var def = DefDatabase<ThingDef>.GetNamedSilentFail(defName);
            if (def == null) return;
        ...
        public static void ScanForThings()
        {
            if (thingDefsToLookFor == null)
            {
                ...
                foreach (var defName in powerLevels.Keys)
                {
                    ...
                    if (def != null) thingDefsToLookFor.Add(def);
                }
            }
            ...
            foreach (var map in Find.Maps)
            {
                ...
                foreach (var def in AutodoorDefs)
                {
                    ...
                    foreach (var building in autoDoorsFound) Autodoors.Add(building);
                }
                ...
                if (rimfactoryIsLoaded)
                {
                    foreach (var def in rimfactoryAssemblerDefs)
                    {
                        ...
                        foreach (var building in rimfactoryBuildings) RimfactoryBuildings.Add(building);
                    }
                }
                ...
                var turrets = from Building turret in map.listerBuildings.allBuildingsColonist
                              ...
                              select turret;
                foreach (var building in turrets) Turrets.Add(building as Building_Turret);
        ...
        private static void ScanExternalReservable()
        {
            ...
            foreach (var def in buildingDefsReservable)
            {
                foreach (var map in Find.Maps)
                {
                    ...
                    foreach (var building in buildings) reservableBuildings.Add(building);
                }
            }
        }
        ...
        public static void EvaluateBeds()
        {
            ...
            {
                if (medicalBed?.Map == null) continue;

                var occupied = false;
                foreach (var unused in medicalBed.CurOccupants) occupied = true;
        ...
        {
            if (!rimfactoryIsLoaded) return;

            foreach (var rimfactoryBuilding in RimfactoryBuildings)
            {
                if (rimfactoryBuilding?.Map == null) continue;
                if (buildingsInUseThisTick.Contains(rimfactoryBuilding)) continue;

                var inspectStringSplitted = rimfactoryBuilding.GetInspectString().Split('\n');
                foreach (var row in inspectStringSplitted)
                {
                    if (!row.Contains("(") || row.Contains("W")) continue;
            ...
            foreach (var building in buildingsToModifyPowerOn)
            {
                ...
                if (buildingsInUseThisTick.Contains(building)) continue;

                var interactionSpotBuilding = building.InteractionCell.GetFirstBuilding(building.Map);
                if (interactionSpotBuilding?.def.thingClass.FullName == null) continue;
                if (!interactionSpotBuilding.def.thingClass.FullName.StartsWith("ProjectRimFactory")) continue;
        ...
        private static void updateRepowerDefs()
        {
            ...
            foreach (var def in defs)
            {
                ...
                if (def.poweredWorkbench) registerPowerUserBuilding(namedDef.defName, def.lowPower, def.highPower);
                if (def.poweredReservable) registerExternalReservable(namedDef.defName, def.lowPower, def.highPower);
                if (def.scheduledPower) ScheduledBuildingsDefs.Add(namedDef);
        ...
        {
            if (typeof(Building_Door).IsAssignableFrom(def.thingClass)) return true;
        ...
        private static void UpdateDefinitions()
        {
            ...
            var settings = TurnOnOffMod.Instance?.Settings;
            var lowPower = -10f;
            if (settings != null) lowPower = settings.lowValue * -1f;

            var highPowerMultiplier = settings?.highMultiplier ?? 2.5f;
            if (highPowerMultiplier <= 0) highPowerMultiplier = 0.001f;

            var doorPowerMultiplier = settings?.doorMultiplier ?? 10f;
            if (doorPowerMultiplier <= 0) doorPowerMultiplier = 0.001f;
            ...
            foreach (var tvDef in DefDatabase<ThingDef>.AllDefsListForReading.Where(tvDef =>
                         tvDef.building?.joyKind == DefDatabase<JoyKindDef>.GetNamed("Television")))
            {
            ...
            foreach (var def in DefDatabase<ThingDef>.AllDefsListForReading)
            {
                ...
                if (repowerVanilla.Any(a => a[0] == def.defName))
                {
                    ...
                    {
                        registerPowerUserBuilding(def.defName, -Convert.ToInt32(repowerSetting[1]), -Convert.ToInt32(repowerSetting[2]));
                    }
                    else
                    {
                ...
                if (powerProps == null || !typeof(CompPowerTrader).IsAssignableFrom(powerProps.compClass)) continue;
                if (powerLevels.ContainsKey(def.defName)) continue;

                if (specialCases.Contains(def.defName))
                {
                    registerSpecialPowerTrader(def.defName, lowPower, powerProps.PowerConsumption * highPowerMultiplier * -1);
                    continue;
                }
                ...
                if (isDoorType(def))
                {
                    ...
                    registerSpecialPowerTrader(def.defName, lowPower, powerProps.PowerConsumption * doorPowerMultiplier * -1);
                    continue;
                }
        ...
        public static void EvalScheduledBuildings()
        {
            foreach (var building in scheduledBuildings)
            {
                ...
                if (comp.Allowed) buildingsInUseThisTick.Add(building);
            }
        }

        public static void EvalHydroponicsBasins()
        {
            if (selfLitHydroponicsIsLoaded) return;
            foreach (var basin in HydroponcsBasins)
            {
                ...
                foreach (var tile in basin.OccupiedRect())
                {
                    ...
                    {
                        if (thing is not Plant) continue;
        ...
        public static void EvalTurrets()
        {
            ...
            {
                if (turret?.Map == null) continue;
                if (turret.CurrentTarget == LocalTargetInfo.Invalid) continue;
        ...
        public static bool HasEnoughPower(Building_WorkTable table)
        {
            ...
            if (!settings.blockUseWhenLowPower) return true;
            if (!powerLevels.ContainsKey(table.def.defName)) return true;
            if (table.CanWorkWithoutPower) return true;
            if (buildingsThatWereUsedLastTick.Contains(table)) return true;

            var powerTrader = table.GetComp<CompPowerTrader>();
            if (powerTrader?.PowerNet == null) return true;
            ...
            {
                LogMessage($"Enough power: {table.def.label} requires {currentPowerNeed}, current gainrate: {currentGainRate}");
                return true;
            }

            if (currentSavedEnergy > currentPowerNeed * 500)
            {
                LogMessage($"Enough power: {table.def.label} requires {currentPowerNeed * 500} saved, current saved power: {currentSavedEnergy}");
                return true;
            }

            LogMessage($"Not enough power: {table.def.label} requires {currentPowerNeed} gaining or {currentPowerNeed * 500} saved, current saved power: {currentSavedEnergy}, current gainrate: {currentGainRate}");
